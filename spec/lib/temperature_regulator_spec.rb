  ...
  describe '#regulate' do
    ...
    context '前回の設定温度が最低設定温度の場合' do
      ...
      it '最低設定温度と理由が返される（クランプされる）' do
        expect(regulate).to eq({ temperature: described_class::MIN_SET_TEMPERATURE, reason: :lowered })
      end
    end
    ...
    context '前回の設定温度が最高設定温度の場合' do
      ...
      it '最高設定温度と理由が返される（クランプされる）' do
        expect(regulate).to eq({ temperature: described_class::MAX_SET_TEMPERATURE, reason: :raised })
      end
    end
  end
end
